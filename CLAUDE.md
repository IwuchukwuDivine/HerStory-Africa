...
- **OG images**: one Satori template, `app/components/OgImage/Card.satori.vue`, renders every route (full-bleed portrait under a dark veil, or a terracotta ground with the continent watermark). Each page calls `defineOgImage("Card", …)`; the module fills `description` from the page meta unless the page passes one. Fonts come from the single global `fonts.families` entry and the files in `public/fonts/`; the template's `fontWeight` binding must stay props-dependent so the module keeps the 800/900 and italic faces.
